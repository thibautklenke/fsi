/* ---------------------------------------------------------------------
 *
 * Copyright (C) 2025 Sebastian Kinnewig
 *
...
template <int dim, typename Number, typename MemorySpace>
void
OptimizedFROSchPreconditioner<dim, Number, MemorySpace>::distribute_vector (
  ...
  // turn the vector into a xpetra vector:
  Teuchos::RCP<LA::XpetraTypes::MultiVectorType<Number, MemorySpace>> xpetra_global_solution = 
    Xpetra::toXpetra(global_solution.trilinos_rcp());

  // Create the communcation plan, and create the global vector onto the overlapping one
  Teuchos::RCP<Xpetra::Import<LA::LO, LA::GO, LA::NO<MemorySpace> > > scatter = 
    Xpetra::ImportFactory<LA::LO, LA::GO, LA::NO<MemorySpace> >::Build(
      xpetra_global_solution->getMap(), overlapping_map);
  Teuchos::RCP<LA::XpetraTypes::MultiVectorType<Number, MemorySpace> > xpetra_subdomain_vector = 
    LA::XpetraTypes::MultiVectorFactoryType<Number, MemorySpace>::Build(overlapping_map, 1);
  // Communicate the solution vector onto the subdomain vector:
  xpetra_subdomain_vector->doImport(*xpetra_global_solution, *scatter, Xpetra::ADD);

  // Build a local (= only this MPI rank) vector:
  // First, we need to create a MPI communicator only for this rank:
  Teuchos::RCP<const Teuchos::Comm<LA::LO> > SerialComm = rcp(new Teuchos::MpiComm<LA::LO>(MPI_COMM_SELF));
  // Next, we create the local only map
  Teuchos::RCP<const LA::XpetraTypes::MapType<MemorySpace> > local_subdomain_map = 
    LA::XpetraTypes::MapFactoryType<MemorySpace>::Build(
      Xpetra::UseTpetra,
      overlapping_map->getLocalNumElements(),
      0,
      SerialComm);
  // Next, if one would like to use a Xpetra::Vector<SC,LO,GO,NO> (not used here)
  //Teuchos::RCP<LA::XpetraTypes::MultiVectorType<Number, MemorySpace> > xpetra_local_subdomain_vector = 
  //  LA::XpetraTypes::MultiVectorFactoryType<Number, MemorySpace>::Build(local_subdomain_map, 1);
  // Next, we create the local only deal.II vector:
  Teuchos::RCP<const Tpetra::Map<LA::LO, LA::GO, LA::NO<MemorySpace>>> tpetra_local_subdomain_map =
    Xpetra::toTpetra(local_subdomain_map); 
  //LinearAlgebra::TpetraWrappers::Vector<Number, MemorySpace> local_subdomain_vector(
  //    IndexSet(tpetra_local_subdomain_map), 
  //    MPI_COMM_SELF
  //  );

  // Copy the data into the vector
  const size_t local_length        = xpetra_subdomain_vector->getLocalLength();
  auto subdomain_vector_data       = xpetra_subdomain_vector->getData(0);
  auto local_subdomain_vector_data = local_subdomain_vector.trilinos_rcp()->getData(0);
  for (unsigned int i = 0; i < local_length; ++i)
    local_subdomain_vector[i] = subdomain_vector_data[i];
}
